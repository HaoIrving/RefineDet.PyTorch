...
class RefineDet(nn.Module):
    ...
    def forward(self, x):
        ...
        for (x, ponits, l, c) in zip(
            ...
            dcn_offset  = torch.cat([dcn_offset1, dcn_offset2, dcn_offset3], 1)
            
            adm_loc.append(l(x, dcn_offset).permute(0, 2, 3, 1).contiguous())
            adm_conf.append(c(x, dcn_offset).permute(0, 2, 3, 1).contiguous())
...
def adm_multibox(level_channels, anchor_nums, num_classes):
    ...
    for i in range(len(level_channels)):
        adm_loc_layers += [DeformConv2d(256, anchor_nums[i] * 4, kernel_size=3, padding=1, deformable_groups=3)]
        adm_conf_layers += [DeformConv2d(256, anchor_nums[i] * num_classes, kernel_size=3, padding=1, deformable_groups=3)]
