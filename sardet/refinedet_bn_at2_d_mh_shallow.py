...
class RefineDet(nn.Module):
    ...
    def _init_solo_layers(self):
        ...
        for i in range(head_num):
            last_conv1 = DeformConv2d(self.in_channels[i], self.num_classes - 1, 3, padding=1)
            last_conv2 = DeformConv2d(self.in_channels[i], self.num_classes - 1, 3, padding=1)
            last_conv3 = DeformConv2d(self.in_channels[i], self.num_classes - 1, 3, padding=1)
            grid_cate += [last_conv1, last_conv2, last_conv3]
    ...
    def forward(self, x, img_id=None, img_gt=None):
        ...
        attention_maps = list()
        for (x, ponits, lvl) in zip(tcb_source, adm_points, range(self.lvl_num)):
            ...
            f1 = self.grid_cate[lvl * self.anchor_num    ](x, dcn_offset1)
            f2 = self.grid_cate[lvl * self.anchor_num + 1](x, dcn_offset2)
            f3 = self.grid_cate[lvl * self.anchor_num + 2](x, dcn_offset3)
            cate_feat = f1 + f2 + f3
