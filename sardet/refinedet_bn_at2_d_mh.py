...
try:
    from mmcv.cnn import bias_init_with_prob, ConvModule, build_norm_layer
except:
    from mmdet.models.utils import bias_init_with_prob, ConvModule, build_norm_layer
...
class DeConvModule(nn.Module):
    def __init__(self, 
        ...
        self.conv1 = DeformConv2d(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=padding)
        self.conv2 = DeformConv2d(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=padding)
        self.conv3 = DeformConv2d(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=padding)
    ...
    def forward(self, x, offset1, offset2, offset3):
        f1 = self.conv1(x, offset1)
        f2 = self.conv2(x, offset2)
        f3 = self.conv3(x, offset3)
        f = self.norm(f1+f2+f3)
        f = self.activate(f)
        return f
...
class RefineDet(nn.Module):
    ...
    def forward(self, x, img_id=None, img_gt=None):
        ...
        attention_maps = list()
        for (x, ponits, lvl, cate) in zip(tcb_source, adm_points, range(self.lvl_num), self.grid_cate):
            ...
            x = self.cate_convs[lvl](x, dcn_offset1, dcn_offset2, dcn_offset3)
            cate_feat = cate(x)
    ...
    def init_weights(self, pretrained=None):
        ...
        for m in self.cate_convs:
            normal_init(m.conv1, std=0.01)
            normal_init(m.conv2, std=0.01)
            normal_init(m.conv3, std=0.01)
