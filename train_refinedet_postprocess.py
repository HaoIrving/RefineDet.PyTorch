...
from layers.modules import AttentionFocalLoss
from layers.modules.refinedet_multibox_loss_postprocess import RefineDetMultiBoxLoss
...
parser.add_argument('--model', default='640_vggbn',
...
parser.add_argument('-wof', '--wo_filter', action="store_true", default=False, help=' ')
...
def train():
    ...
    if args.wo_filter:
        from layers.modules.refinedet_multibox_loss_postprocess_wo_filter import RefineDetMultiBoxLoss
    ...
    odm_criterion = RefineDetMultiBoxLoss(cfg['num_classes'], 0.5, True, 0, True, negpos_ratio, 0.5,
                             False, args.cuda, theta=1e-4, use_ARM=True)
    # attention criterion 
    ...
    for iteration in range(start_iter, max_iter):
        ...
        # forward
        attention_maps, confidence_maps, arm_loc, arm_conf, odm_loc, odm_conf = net(images)
        out = (confidence_maps, arm_loc, arm_conf, odm_loc, odm_conf)
